...
export const ManageChallenge = ({
        ...
        if (isFormValid()) {
            try {
                if (createNewChallenge) {
                    ...
                    const createChallengeOnChain = await createChallenge(challengeTeam1, challengeTeam2, challengeAmount);
                    ...
                    console.log("challenge form data with challengeChainId", challengeFormData);
    ...
    const handleSports = (sport, isChecked) => {
        if (isChecked) {
            setChallengeSports("");
        } else {
    ...
    const isFormValid = () => {
        if (!challengeTeam1 || !challengeTeam2 || !challengeAmount || !challengeSports) {
    ...
    const getTeamsByUsername = useMoralisQuery("teams", (query) => query.equalTo("teamAdmin", user.get("username")), [], {
        autoFetch: true,
    });
    ...
    return (
        <Modal open={modalView} onClose={() => toggleModal(false)}>
            <div className="flex flex-col border-2 border-green-100 p-4 items-center">
                <div> {createNewChallenge ? `Create Challenge Against ${team.teamName}` : `Manage Challenge`}</div>
                    ...
                    <>
                        <div className="flex flex-row w-full">
                            <div className="w-1/2 p-2">
                                <span className="h-[60px] my-1 flex justify-end items-center">Choose Team*:</span>
                                <span className="h-[60px] my-1 flex justify-end items-center">Amount*:</span>

                                <span className="h-[60px] my-1 flex justify-end items-center">Message:</span>

                                <span className="h-[120px] my-1 flex justify-end items-center">Sports*:</span>
                            </div>
                            <div className="w-1/2 p-2">
                                <span className="h-[60px] my-1 flex justify-start items-center">
                                    <select required onChange={(e) => setChallengeTeam1(e.target.value)}>
                                ...
                                <span className="h-[120px] my-1 flex justify-start items-center flex-wrap">
                                    {team.teamSportsPreferences.sort().map((sport, i) => {
                                        const isChecked = challengeSports.includes(sport);
                                        return (
                                            <button
                                                key={i}
                                                className={`m-2 px-2 py-1 rounded text-sm  ${isChecked ? "bg-green-100" : "bg-gray-300"}`}
