...
export const ManageTeam = ({ user, team = false, toggleModal, modalView, createNewTeam = false, teamObject = null }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if ((!getTeamsDB.isSaving && !isContractLoading && !contractMessage) || !teamObject.isSaving) router.reload();
        } catch (error) {
            console.error(error);
        }
    };
    ...
    const handleSportsPreferences = (sport, isChecked) => {
        if (isChecked) {
            setTeamSportsPreferences([...teamSportsPreferences].filter((eaSport) => eaSport != sport));
        } else {
                    ...
                    <>
                        <div className="flex flex-row w-full">
                            <div className="w-1/2 p-2">
                                <span className="h-[60px] my-1 flex justify-end items-center">Team Name:</span>

                                <span className="h-[60px] my-1 flex justify-end items-center">Description:</span>

                                <span className="h-[120px] my-1 flex justify-end items-center">Sports Preferences:</span>

                                <span className="h-[60px] my-1 flex justify-end items-center">Team Photo:</span>
                            </div>
                            <div className="w-1/2 p-2">
                        ...
                        <div className="p-2">
                            <div className="flex flex-row justify-center">
                                ...
                                >
                                    <div
                                        className={
                                            "bg-white md:w-6 md:h-6 h-5 w-5 rounded-full shadow-md transform" + (isTeamActive ? toggleClass : null)
                                        }
                                    ></div>
                                </div>
                            </div>
